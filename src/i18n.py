...
TRANSLATIONS = {
    "fr": {
        ...
        "africa.avertissement_general": "⚠ Données à manier avec précaution : contrairement à la France, il n'existe pas de recensement fiable à l'échelle infracommunale pour la plupart de ces villes (population WorldPop = estimation modélisée, pas un comptage), la couverture OpenStreetMap des équipements est très inégale selon les villes et les quartiers, et plusieurs GTFS de ce catalogue datent de 2018-2020 et ne sont plus maintenus (cf. data/GTFS_Africa/_provenance.json). Les résultats (grille de population, équipements, accessibilité) sont des ordres de grandeur, pas des chiffres de référence.",
    ...
    **Contributeur :** Antoine Chèvre 🐐 (et claude.ai....)
    ...
    """,
        ...
        "equipements.header_stats": "Nombre d'équipements par couche",
        ...
        "accessibilite.pas_de_ttm": "Pas de matrice de temps de trajet calculée pour {reseau} : lancez d'abord index_accessibility_notebook_abidjan.ipynb (ou l'équivalent) pour ce réseau — cette page se contente de lire un résultat déjà calculé (le calcul r5py est trop lourd pour tourner dans l'app).",
    ...
    "en": {
        ...
        "africa.title": "🌍 GTFS Analysis Africa",
        "africa.avertissement_general": "⚠ Handle these results with caution: unlike France, there is no reliable sub-municipal census for most of these cities (WorldPop population is a modeled estimate, not a headcount), OpenStreetMap facility coverage varies a lot between cities and neighborhoods, and several GTFS files in this catalog date back to 2018-2020 and are no longer maintained (cf. data/GTFS_Africa/_provenance.json). Results (population grid, facilities, accessibility) are orders of magnitude, not reference figures.",
    ...
    **Contributor:** Antoine Chèvre 🐐 (and claude.ai....)
    ...
    """,
        ...
        "equipements.header_stats": "Number of facilities per layer",
        ...
        "accessibilite.pas_de_ttm": "No travel time matrix computed for {reseau}: first run index_accessibility_notebook_abidjan.ipynb (or equivalent) for this network — this page only reads an already-computed result (the r5py computation is too heavy to run inside the app).",
